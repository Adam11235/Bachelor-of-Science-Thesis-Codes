...
# Wyłącz logowanieA Optuny dla każdej próby, aby uniknąć nadmiernego outputu
...
def load_data(path=r"c:\Users\the30\OneDrive\Pulpit\inz\wetransfer_inzynierka_2025-12-21_1420\inzynierka\moj_dataset\dane_bez_cev.csv"):
    ...
    try:
        ...
        # Definicja kolumn cech i celu na podstawie Twojego notatnika
        feature_columns = ['~ C', '~ Cr','~ Ni','~ Mo','~ Mn','~ Al']
        target_columns = ['minYield', 'minTensileStrenght', 'minElongation']
        
        # Konwersja na numeryczne (z komórki 195)
        # Dodajemy '~ Al' do feature_columns, ponieważ jest w Twoim notatniku w X_cols (komórka 194)
        # ale brakowało go w definicji w komórce 198
        ...
        # Poprawiona lista cech (zgodna z komórką 198 i 194)
        final_feature_columns = ['~ C', '~ Cr','~ Ni','~ Mo','~ Mn','~ Al']
...
def objective(trial, X_train, y_train, X_test, y_test):
    ...
    if model_type == 'SVR':
        ...
        # 2. Regresja Losowego Lasu (Random Forest)
